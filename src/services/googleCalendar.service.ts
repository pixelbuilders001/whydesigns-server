  ...
  async createEvent(eventData: CalendarEventData): Promise<CalendarEvent | null> {
    ...
    try {
      ...
      const event = {
        ...
        // Guest will receive meeting link via confirmation email instead
        reminders: {
          useDefault: false,
          overrides: [
      ...
      // Try to create event with conference data first
      let response;
      try {
        response = await this.calendar.events.insert({
          ...
          sendUpdates: 'none', // No attendees to notify - guest gets email from app instead
        });
      } catch (conferenceError: any) {
        // If conference creation fails, try without conferenceData
        console.warn('⚠️  Could not create Google Meet link, creating event without it:', conferenceError.message);
      ...
      if (meetingLink) {
        ...
        console.warn('⚠️  No meeting link generated by Google Calendar');
      }
